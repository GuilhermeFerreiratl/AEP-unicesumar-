# AEP-unicesumar-
# AEP 1ª ETAPA - Programa Saúde na Escola (PSE)
**Tema:** Controle de Restrição Alimentar e Acompanhamento de Crianças com Autismo

### 👥 Integrantes
-Guilherme Moraes Ferreira - RA:26011382-2 
-Gabriel ibaniz Volpato- RA:26011647-2 
-Bruno de Souza Garcia- RA:26010743-2 


### 📄 Entregável Principal - Documento da 1ª Etapa
Contém: Produção textual, requisitos, fluxogramas, pseudocódigos e planejamento de sprints.


### 🖼️ Fluxogramas e Diagramas

#### 1. Fluxograma Geral do Sistema (Menu Principal)


#### 2. Fluxograma Detalhado (Cadastro de Restrição e Acompanhamento)
Fluxograma Detalhado.

### ✅ Funcionalidades do Sistema Planejado
- Cadastro de Aluno
- Cadastro de Restrição Alimentar
- Precisa de acompanhamento? Sim / Não
- Tem acompanhamento? Sim / Não
- Qual tipo de acompanhamento?
- Pesquisa e Geração de Resumo para o PSE

